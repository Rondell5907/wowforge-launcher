...
// ── Session ───────────────────────────────────────────────────────────────────
function readSession()  { try { if (fs.existsSync(SESSION_FILE)) return JSON.parse(fs.readFileSync(SESSION_FILE,'utf8')); } catch{} return null; }
function writeSession(d){ ensureConfig(); fs.writeFileSync(SESSION_FILE, JSON.stringify(d), 'utf8'); }
function clearSession() { if (fs.existsSync(SESSION_FILE)) fs.unlinkSync(SESSION_FILE); }

// ── Window ────────────────────────────────────────────────────────────────────
let mainWindow;
function createWindow() {
  mainWindow = new BrowserWindow({
    width: 1100, height: 650, frame: false, resizable: false,
...
async function srpVerify(username, password, row) {
  ...
  const h1 = crypto.createHash('sha1').update(Buffer.from(`${username.toUpperCase()}:${password.toUpperCase()}`, 'utf8')).digest();
  const h2 = crypto.createHash('sha1').update(Buffer.concat([saltBuf, h1])).digest();
  const h2BigInt = BigInt('0x' + Buffer.from(h2).reverse().toString('hex'));
  const g = 7n, N = BigInt('0x894B645E89E1535BBDAD5B8B290650530801B18EBFBF5E8FAB3C82872A3E9BB7');
  let base = g % N, exp = h2BigInt, result = 1n;
  while (exp > 0n) { if (exp % 2n === 1n) result = (result * base) % N; exp = exp / 2n; base = (base * base) % N; }
  let hex = result.toString(16); if (hex.length % 2) hex = '0' + hex;
  const computedBuf = Buffer.alloc(32); Buffer.from(hex, 'hex').reverse().copy(computedBuf);
...
ipcMain.handle('config:write',   (_e, updates) => writeConfig(updates));
...
ipcMain.handle('dialog:browseExe', async () => {
...
ipcMain.handle('db:connect', async () => {
  ...
  try { const conn = await mysql.createConnection(DB_CONFIG); await conn.end(); return { ok: true }; }
...
ipcMain.handle('auth:login', async (_e, { username, password, rememberMe }) => {
  ...
  try {
    ...
    const row = rows[0];
    if (!await srpVerify(username, password, row)) return { ok: false, error: 'Incorrect password.' };
    if (rememberMe) writeSession({ username, password }); else clearSession();
...
ipcMain.handle('chars:fetch', async (_e, { accountId }) => {
  ...
  try {
    ...
    const [rows] = await conn.execute(
      'SELECT name, race, class, gender, level, zone, online FROM characters WHERE account = ? ORDER BY online DESC, level DESC LIMIT 10',
      [accountId]
    );
    await conn.end();
    return { ok: true, chars: rows };
  } catch (e) { return { ok: false, chars: [], error: e.message }; }
});

ipcMain.handle('articles:fetch', async () => {
  const mysql = getMysql();
  if (!mysql) return { ok: false, articles: [], error: 'mysql2 missing' };
  let conn;
  try {
    ...
    const [rows] = await conn.execute(`
      SELECT id,
        ...
        timestamp
      FROM articles ORDER BY timestamp DESC LIMIT 20
    `);
    return { ok: true, articles: rows ?? [] };
  } catch (e) { return { ok: false, articles: [], error: e.message }; }
  finally { if (conn) try { await conn.end(); } catch {} }
});

ipcMain.handle('wow:launch', async () => {
  const cfg = readConfig();
  const exePath = cfg.wowInstallPath;
  if (!fs.existsSync(exePath)) return { ok: false, error: `Wow.exe not found at: ${exePath}` };
